...
two_player_button = Button(300, 400, 100, 50, "Two Player")
ai_button = Button(450, 400, 100, 50, "AI Mode")
...
def ai_move(player):
    ...
    # Filter out opposite direction to avoid instant collision
    ...
    for dx, dy in directions:
        ...
        # Check if direction leads to immediate collision
    ...
    # Choose a safe direction, or random if none safe
    if safe_directions:
        # Prioritize direction with most open space (simple lookahead)
        best_score = -1
        best_dir = random.choice(safe_directions)  # Fallback
        for dx, dy in safe_directions:
            steps = 0
            x, y = player.x + dx, player.y + dy
            while 0 <= x < WIDTH and 0 <= y < HEIGHT:
                if (x, y) in game_state.player1.trail[:-1] + game_state.player2.trail[:-1]:
                    break
                steps += 1
                x += dx
                y += dy
            if steps > best_score:
                best_score = steps
                best_dir = (dx, dy)
        player.dx, player.dy = best_dir
    else:
        # If no safe direction, continue current direction (will likely die)
        pass
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_d and game_state.player1.dx != -GRID_SIZE:
                game_state.player1.dx, game_state.player1.dy = GRID_SIZE, 0
            # Player 2 controls (Arrows) - only in two-player mode
...
def reset_game():
    ...
    game_state.player2 = Player(700, 300, -GRID_SIZE, 0, RED, game_state.player2_name if game_state.game_mode == "two_player" else "AI")
...
async def main():
    ...
    def update_loop():
        if game_state.state == "menu":
            ...
            handle_game_input()
            if game_state.game_mode == "ai" and not game_state.game_over:
                ai_move(game_state.player2)
            ...
            if game_state.game_over:
                winner = None
                if check_collision(game_state.player1) and not check_collision(game_state.player2):
                    winner = game_state.player2.name
                elif check_collision(game_state.player2) and not check_collision(game_state.player1):
                    winner = game_state.player1.name
                elif check_collision(game_state.player1) and check_collision(game_state.player2):
                    winner = "No one"
                text = font.render(f"Game Over! {winner} wins! Press R to Restart", True, WHITE)
